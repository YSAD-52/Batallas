<template>
  <div class="battle-container">
    ...
    <div class="pokemon-wrapper">
      ...
      <div class="pokemon-side">
        <div v-if="pokemon1.data">
          <h3>{{ pokemon1.data.name | capitalize }}</h3>
          <img :src="pokemon1.data.sprites.front_default" alt="Imagen de {{ pokemon1.data.name }}" />
          <p>HP: {{ pokemon1.hp }}</p>
        </div>
        <div v-else>
      ...
      <div class="pokemon-side">
        <div v-if="pokemon2.data">
          <h3>{{ pokemon2.data.name | capitalize }}</h3>
          <img :src="pokemon2.data.sprites.front_default" alt="Imagen de {{ pokemon2.data.name }}" />
          <p>HP: {{ pokemon2.hp }}</p>
        </div>
        <div v-else>
    ...
    <div class="attack-buttons" v-if="pokemon1.data && pokemon2.data && pokemon1.hp > 0 && pokemon2.hp > 0">
      <button @click="attack(pokemon1, pokemon2)">¡{{ pokemon1.data.name | capitalize }} ataca!</button>
      <button @click="attack(pokemon2, pokemon1)">¡{{ pokemon2.data.name | capitalize }} ataca!</button>
    </div>

    <!-- Mostrar el ganador -->
    <div v-else-if="winner">
      <h2>{{ winner }} es el ganador!</h2>
    </div>
  </div>
</template>
